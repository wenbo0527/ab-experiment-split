...
> 数据来源：`realtime_prebucket.py`（5000 用户 × 100 次抽样）

**核心思想**：实时场景的关键不是"放弃批量预分桶"，而是"用预分桶表 + 按组均衡消耗"。

| 方案 | 平均偏差 | < 1% 通过率 | 关键机制 |
|---|---|---|---|
| **P1 用户池预留** | **0.00%** | **100%** | 预分桶 N 槽位，按组轮转消耗 |
| **P2 动态扩容** | **0.00%** | **100%** | 池子用满 80% 触发异步扩容 |
| P3 分层路由（直接 hash） | 7.04% | 0% | 预分桶表 + 实时 hash（错误用法） |

**关键洞察**：
- 预分桶表（bucket→group）只是第一步
- **必须按组均衡消耗槽位**，否则 hash 分布的不均会传导到组间偏差
- 用户池预留 + 轮转分配 = 0% 偏差 + O(1) 查询 + 100% 一致性
...
**实战推荐**：
- 实验规模可预估：P1 用户池预留（5-10 倍预估容量）
- 规模不确定：P2 动态扩容（初始 1 万，80% 触发）
- 超大规模（百万级）：P1 + Redis 部署 + 异步扩容
